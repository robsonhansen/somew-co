...
export function VideoModal({ isOpen, onClose, videoUrl, videoTitle = "Project Video" }: VideoModalProps) {
  ...
  const videoUrlComParametrosModal = `${videoUrl}${
    videoUrl.includes("?") ? "&" : "?"
  }autoplay=1&loop=0&autopause=0&muted=0&controls=1&title=1&byline=1&portrait=1`;

  return (
    <div
      ref={modalRef}
      className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-75 px-32 py-20"
      onClick={handleOverlayClick}
      aria-modal="true"
      role="dialog"
      aria-labelledby="videoModalTitle"
    >
      <div className="relative bg-black w-full h-full aspect-video shadow-xl rounded-lg">
        <button
          onClick={onClose}
