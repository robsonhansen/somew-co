...
// Se você não quiser depender do "slug", apenas renderize todos os projetos ou um projeto fixo.
export default function ProjectProfile() {
  const { language } = useLanguage();

  // Seleção fixa de projeto (ou todos)
  const selectedProject = projectsData["campanha-verao"]; // Pode mudar para qualquer chave que deseje exibir.
  // Ou, se preferir exibir todos, pode desconsiderar a variável `selectedProject` e iterar sobre todos os projetos.

  return (
    <section className="py-16 bg-white">
      <div className="container mx-auto px-4">
        <h1 className="text-4xl font-bold mb-6">{selectedProject.name}</h1>
        <p className="text-xl mb-12 max-w-3xl">
          {language === "pt" ? selectedProject.description.pt : selectedProject.description.en}
        </p>
